...
date = datetime(2020,3,11) + caldays(0:T);
...
b_mean = mean(b,2); b_mean = reshape(b_mean,6,T+1);
...
b_25perc = prctile(b,25,2); b_25perc = reshape(b_25perc,6,T+1);
b_75perc = prctile(b,75,2); b_75perc = reshape(b_75perc,6,T+1);
Re_25perc = prctile(Re,25,2); Re_25perc = reshape(Re_25perc,6,T+1);
Re_75perc = prctile(Re,75,2); Re_75perc = reshape(Re_75perc,6,T+1);
...
for i = 1:n_tracts
    ...
    plot(date,mean(I_plot)+mean(R_I_plot),'LineWidth',width)
    ylabel('I+R')  
...
% plot of S, E, I, and R
figure(4)
set(gcf, 'Position',  [200, 200, 1400, 620])
for i = 1:n_tracts
    subplot(4,6,0*n_tracts+i)
...
xline(datetime(2020,3,24),'--', 'LineWidth',width,'Color','black');
xline(datetime(2020,5,14),'--', 'LineWidth',width,'Color','black');
xline(datetime(2020,6,5), '--','LineWidth',width,'Color','black');
xline(datetime(2020,6,26),'--', 'LineWidth',width,'Color','black');
xline(datetime(2020,7,16),'--', 'LineWidth',width,'Color','black');
lgd = legend('region 1','region 2','region 3','region 4','region 5','region 6');
lgd.FontSize = 24;
hold off
...
% 7-day average effective reproductive number
Re_title = ["Region 1", "Region 2", "Region 3", "Region 4", "Region 5", "Region 6"];
figure
set(gcf, 'Position',  [200, 200, 1500, 500])
for i = 1:n_tracts
    subplot(2,ceil(n_tracts/2),i)
    plot(date,Re_7average(i,:),'LineWidth',width,'Color',[201,35,35]/255)
    hold on;

    inBetween = [Re_25perc_7average(i,:), fliplr(Re_75perc_7average(i,:))];
    fill([date,flip(date)], inBetween, [201,35,35]/255, 'EdgeColor','none','facealpha',0.25);
    plot(date,ones(T+1,1),'-.','Color','b','LineWidth',width)
    xline(datetime(2020,3,24), 'LineWidth',width,'Color',[245,163,56]/255);
    xline(datetime(2020,5,14), 'LineWidth',width,'Color',[247,217,21]/255);
    xline(datetime(2020,6,5), 'LineWidth',width,'Color',[21,247,179]/255);
    xline(datetime(2020,6,26), 'LineWidth',width,'Color',[21,202,247]/255);
    xline(datetime(2020,7,16), 'LineWidth',width,'Color',[185,107,238]/255);
    hold off
    ylim([0,7])
    ...
    set(gca,'Fontsize',24)
    if(i==n_tracts)
        lgd = legend({'mean','IQR','R_e=1'},'Location','northwest','Orientation','horizontal');
        lgd.FontSize = 24;
    end
end

% saveas(gcf,'plots\Re_7average.jpg')
